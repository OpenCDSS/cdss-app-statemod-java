@echo off
rem Configure and run Eclipse Neon 3 32-bit Windows for standard StateMod Java development environment.
...
rem - can probably use a 64-bit JRE for StateMod Java but are trying to be efficient with
rem   reusing already installed software

rem It is assumed that the development environment follows the standard folder structure, for example:
rem - %USERPROFILE%\cdss-dev\StateMod-Java\git-repos\cdss-app-statemod-java
...
rem - additional "standard" installation locations can be added for StateMod Java developers
set eclipseExe=""
...
rem - this is 32-bit given StateMod Java is not yet 64-bit
...
title Eclipse configured for StateMod Java development
